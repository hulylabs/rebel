...
fn test_alloc_string() {
    ...
    let string_series = memory.alloc_string(hello).expect("String allocation failed");
    
    // Verify string length
    let length = memory.len(string_series).expect("Failed to get string length");
...
fn test_push_all_function() {
    let mut memory = setup_memory();
    let series = memory.alloc::<Value>(10).expect("Failed to allocate series");
...
fn test_value_conversions() {
    ...
    // Test string value
    let str_series = memory.alloc_string("test").expect("String allocation failed");
...
fn test_capacity_limits() {
    ...
    let series = memory.alloc::<Value>(requested_capacity).expect("Failed to allocate series");
    ...
    assert!(matches!(result, Err(MemoryError::StackOverflow)), 
            "Push should fail with StackOverflow after {} successful pushes", pushes_allowed);
...
fn test_any_word_creation() {
    ...
    let symbol = memory.alloc_string("test").expect("String allocation failed");
...
fn test_capacity_api() {
    ...
    let value_series = memory.alloc::<Value>(requested_capacity).expect("Failed to allocate Value series");
    let value_capacity = rebel::mem::capacity(&memory, value_series).expect("Failed to get Value capacity");
    
    // u32 (each u32 is 4 bytes = 1 word)
    let u32_series = memory.alloc::<u32>(requested_capacity).expect("Failed to allocate u32 series");
    let u32_capacity = rebel::mem::capacity(&memory, u32_series).expect("Failed to get u32 capacity");
    
    // u8 (4 u8s fit in 1 word)
    let u8_series = memory.alloc::<u8>(requested_capacity * 4).expect("Failed to allocate u8 series");
    let u8_capacity = rebel::mem::capacity(&memory, u8_series).expect("Failed to get u8 capacity");
    ...
    let value_size = rebel::mem::block_size_in_words(&memory, value_series).expect("Failed to get block size");
