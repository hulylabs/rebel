...
fn debug_drain_popping() {
    ...
    println!("Series length after pushing: {}", memory.len(series).unwrap());
    ...
    println!("Original series length after drain: {}", memory.len(series).unwrap());
...
fn debug_capacity() {
    ...
    for capacity in 1..6 {
        ...
        let series = memory.alloc::<Value>(capacity).expect("Failed to allocate series");
        
        // Get actual capacity
        let actual_capacity = rebel::mem::capacity(&memory, series).expect("Failed to get capacity");
        let block = memory.get::<rebel::mem::Block>(series.address()).expect("Failed to get block");
        
        println!("\nRequested capacity: {}, Actual capacity: {}", capacity, actual_capacity);
        ...
        println!("Successfully pushed {} values with capacity {}", pushed, capacity);
    }
}
...
fn debug_push_all_function() {
    ...
    println!("Series length after push_all: {}", memory.len(series).unwrap());
    ...
    println!("Series length after popping all: {}", memory.len(series).unwrap());
}