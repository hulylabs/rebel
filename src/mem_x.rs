    ...
    T: Default + Copy,
{
    ...
    pub fn push_resolved(&mut self, item: T, domain: &mut Domain<T>) -> Option<()> {
        domain
    ...
    pub fn push_domain<D>(&mut self, item: T, domain_provider: &mut D) -> Option<()>
    where
        D: GetDomain<T>,
    {
        let domain = domain_provider.get_domain_mut();
        self.push_resolved(item, domain)
    }

    pub fn push<D>(&mut self, item: T, domain_provider: &mut D) -> Option<()>
    where
        D: GetDomain<T>,
    {
        self.push_domain(item, domain_provider)
    }

    pub fn push_all<D>(&mut self, items: &[T], domain_provider: &mut D) -> Option<()>
    where
        D: GetDomain<T>,
    {
        let domain = domain_provider.get_domain_mut();
    ...
    pub fn trim_after<'a, D>(&mut self, offset: Word, domain_provider: &'a mut D) -> Option<&'a [T]>
    where
        D: GetDomain<T>,
    {
        let domain = domain_provider.get_domain_mut();
    ...
    pub fn pop_resolved(&mut self, domain: &mut Domain<T>) -> Option<T> {
    ...
    pub fn pop_domain<D>(&mut self, domain_provider: &mut D) -> Option<T>
    where
        D: GetDomain<T>,
    {
        let domain = domain_provider.get_domain_mut();
        self.pop_resolved(domain)
    }

    pub fn pop<D>(&mut self, domain_provider: &mut D) -> Option<T>
    where
        D: GetDomain<T>,
    {
        self.pop_domain(domain_provider)
    }
}
...
impl Memory {
    #[cfg(test)]
    pub fn values_mut(&mut self) -> &mut Domain<VmValue> {
        &mut self.values
    }
...
impl Memory {
    ...
    pub fn init(&mut self) -> Option<()> {
        self.values = Domain::new(0x20000); // Increase domain capacity
        // Allocate stack data at the end of the domain to avoid overlap
        let stack_data = Addr::new(self.values.items.len() as Word - 256);
        self.stack = Block::new(256, 0, stack_data);
        self.values.len = stack_data.0; // Adjust domain length to leave space for allocations

        let op_stack_data = Addr::new(self.words.items.len() as Word - 128);
        self.op_stack = Block::new(128, 0, op_stack_data);
        self.words.len = self.words.items.len() as Word; // Adjust domain length
    ...
    pub fn stack_push(&mut self, value: VmValue) -> Option<()> {
        let domain = &mut self.values;
        self.stack.push_resolved(value, domain)
    }

    pub fn stack_pop(&mut self) -> Option<VmValue> {
        let domain = &mut self.values;
        self.stack.pop_resolved(domain)
    }
    ...
    pub fn alloc_empty_block(&mut self, cap: Word) -> Option<Addr<Block<VmValue>>> {
        ...
        self.blocks.push(Block::new(cap, 0, data))
    }
    ...
    pub fn alloc_string(&mut self, s: &str) -> Option<Addr<Block<u8>>> {
        let data = self.bytes.push_all(s.as_bytes())?;
        let len = s.len() as Word;
    ...
    pub fn begin(&mut self) -> Option<()> {
        ...
        let domain = &mut self.words;
        self.op_stack.push_resolved(stack_len, domain)
    }

    pub fn end(&mut self) -> Option<Addr<Block<VmValue>>> {
        let word_domain = &mut self.words;
        let offset = self.op_stack.pop_resolved(word_domain)?;
        let items = self.stack.len().checked_sub(offset)?;
        let block = self.alloc_empty_block(items)?;
        let (stack, blocks, values) = (&mut self.stack, &mut self.blocks, &mut self.values);
        let dest_block = blocks.get_item_mut(block)?;
        stack.move_to(dest_block, items, values)?;
        Some(block)
    }
}
...
impl Memory {
    ...
    pub fn get_block_item(
        ...
        block.get_item(index, &self.values)
    }
...
impl Collector for Memory {
    ...
    fn string(&mut self, string: &str) -> Option<()> {
        ...
        let domain = &mut self.values;
        self.stack.push_resolved(string, domain)
    }
    ...
    fn integer(&mut self, value: i32) -> Option<()> {
        let domain = &mut self.values;
        self.stack.push_resolved(VmValue::Int(value), domain)
    }
    ...
    fn end_block(&mut self) -> Option<()> {
        ...
        let domain = &mut self.values;
        self.stack.push_resolved(block, domain)
    }
